...
as
	...
	select distinct top 1 @otherUserId = UserId from ExternStorageStats where ArticleId = @ArticleId and UserId is not null
...
as
	...
	exec ResetArticleStats @articleId, @userId
...
as
	...
	begin
		...
		begin
			...
			delete from ArticleItemDatas where ArticleDataId = @id
			delete from ArticleDatas where Id = @id
		end
		fetch next from dupCur into @id, @code, @originalCount, @orderCount
	end
...
as
	...
	begin
		if @@FETCH_STATUS <> -2
		begin
			exec ResetArticleStats @id, null
		end
		fetch next from articleCur into @id
	end
