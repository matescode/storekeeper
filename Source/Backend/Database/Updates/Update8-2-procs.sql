...
as
	...
	begin
		...
		begin
			...
			delete from ArticleCounts where ExternalId = @externalId
			insert into ArticleCounts (Id, Code, ExternalId, InternalStorage, OriginalCount, OrderCount)
			values (NEWID(), @code, @externalId, @storage, @originalCount, @orderCount)
		end
		fetch next from dataCur into @externalId, @name, @type, @code, @specCode, @originalCount, @storage, @pPrice, @sPrice, @order, @orderCount
	end
...
as
	...
	select Code, sum(OriginalCount) as OriginalCount, sum(OrderCount) as OrderCount from ArticleCounts 
	where Code in (select Code from Articles where Updated = 1)
	group by Code
	...
	fetch next from countCur into @code, @origCount, @ordCount

	while @@FETCH_STATUS <> -1
	begin
		 if @@FETCH_STATUS <> -2
		 begin
			update Articles set OriginalCount = @origCount, OrderCount = @ordCount where Code = @code
		 end
		 fetch next from countCur into @code, @origCount, @ordCount
	end
